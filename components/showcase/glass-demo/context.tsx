...
import type { GlassDemoState, Task, AppSettings } from './types';

const initialTasks: Task[] = [
  { id: 1, title: 'Design Review', description: 'Review new component designs', emoji: '🎨', completed: false, priority: 'high' },
  { id: 2, title: 'Write Tests', description: 'Add unit tests for utils', emoji: '🧪', completed: true, priority: 'medium' },
  { id: 3, title: 'Deploy Staging', description: 'Push latest build to staging', emoji: '🚀', completed: false, priority: 'high' },
  { id: 4, title: 'Update Docs', description: 'Update API documentation', emoji: '📝', completed: false, priority: 'low' },
];
...
export function GlassDemoProvider({ children }: { children: React.ReactNode }) {
  ...
  return (
    <GlassDemoContext.Provider value={value}>
      {children}
    </GlassDemoContext.Provider>
  );
}
